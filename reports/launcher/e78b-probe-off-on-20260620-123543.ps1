...
function GetWorkerSignalsConfig(){
  $cfgPath=Join-Path $WorkerSyncDir "worker-signals-config.json"
  $cfg=ReadJsonSafe $cfgPath
  if($null -eq $cfg){return [pscustomobject]@{worker_signals_enabled=$true;mode="ON";reason="missing_config_default_on"}}
  return $cfg
}
function WorkerSignalsEnabled(){
  $cfg=GetWorkerSignalsConfig
  if($cfg.PSObject.Properties.Name -contains "worker_signals_enabled"){return [bool]$cfg.worker_signals_enabled}
  return $true
}
function GetLocalSignals(){if(-not (WorkerSignalsEnabled)){return [pscustomobject]@{heartbeat=$null;project_signal=$null;enabled=$false}};[pscustomobject]@{heartbeat=(ReadJsonSafe (Join-Path $WorkerSyncDir "worker-heartbeat-latest.json"));project_signal=(ReadJsonSafe (Join-Path $WorkerSyncDir "project-signal-latest.json"));enabled=$true}}
...
function ShowProjects(){
  ...
  foreach($p in $items){$rows.Add((ProjectSignal $p $localSignals))}
  $total=$rows.Count
  ...
  foreach($s in $rows){
    ...
    $line=("{0,2} {1,-36} {2,-24} {3} {4,-7} {5,-5} {6}" -f $i,(Short $s.name 36),$phase,(Bar $s.score $s.worker),(Short $s.worker 7),(Short $s.block 5),(Short $s.next 28))
  ...
  UX "" "info"
  UX "Enter refresca. Workers ON/OFF en worker-signals-config.json; Obsidian solo visualiza Markdown/YAML." "muted"
  Read-Host "PROYECTOS"
}
...
function ProbeOne(){ $projects=@(GetProjects); $signals=GetLocalSignals; $demo=$projects|Where-Object{$_.Name -eq "ENGREMIAT_PROJECT_heartbeat_demo"}|Select-Object -First 1; if($null -eq $demo){throw "Demo project not found"}; $row=ProjectSignal $demo $signals; [pscustomobject]@{signals_enabled=$signals.enabled;project=$row.name;phase=$row.phase;score=$row.score;worker=$row.worker;block=$row.block;next=$row.next;source=$row.signal_source;cause=$row.cause} }
$off=ProbeOne
$out=[pscustomobject]@{off=$off}
