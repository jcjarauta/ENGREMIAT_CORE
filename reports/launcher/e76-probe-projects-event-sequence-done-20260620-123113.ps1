...
$signals=GetLocalSignals
$demo=$projects|Where-Object{$_.Name -eq "ENGREMIAT_PROJECT_heartbeat_demo"}|Select-Object -First 1
if($null -eq $demo){throw "Demo project not found by GetProjects"}
$row=ProjectSignal $demo $signals
$render=("{0,2} {1,-36} {2,-24} {3} {4,-7} {5,-5} {6}" -f 1,(Short $row.name 36),([string]$row.phase),(Bar $row.score $row.worker),(Short $row.worker 7),(Short $row.block 5),(Short $row.next 28))
$out=[pscustomobject]@{project=$row.name;phase=$row.phase;score=$row.score;worker=$row.worker;block=$row.block;next=$row.next;source=$row.signal_source;cause=$row.cause;rendered_line=$render}
