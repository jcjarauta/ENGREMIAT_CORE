...
$badChildren=@();foreach($s in $screens){foreach($cid in @($s.children)){if(!$ids.ContainsKey([string]$cid)){$badChildren+=([string]$s.id+"->"+[string]$cid)}}}
if($badChildren.Count -gt 0){$errors+="bad_children:"+($badChildren -join ",")}else{$ok+="children_links_ok"}
...
$readiness=100;if($errors.Count -gt 0){$readiness-=60};if($warn.Count -gt 0){$readiness-=15};if($readiness -lt 0){$readiness=0}
$decision=if($errors.Count -eq 0){"GO_OPERATOR_V2_HEALTHY"}else{"NO_GO_OPERATOR_V2_HEALTHCHECK_FAILED"}
$report=[ordered]@{created_at=(Get-Date).ToString("s");package_id="ENGREMIAT_OPERATOR_V2_HEALTHCHECK_004";stage="E01_CREATE_OPERATOR_V2_HEALTHCHECK_READONLY";decision=$decision;readiness=$readiness;active_root=$root;registry=$registry;engine=$engine;screens=$screens.Count;actions_ready=$actionsReady;actions_planned=$actionsPlanned;errors=$errors;warnings=$warn;ok=$ok;next=if($decision -eq "GO_OPERATOR_V2_HEALTHY"){"E02_CLOSE_HEALTHCHECK_AND_RETURN_TO_ACTIONS_003"}else{"REPAIR_HEALTHCHECK_ERRORS_FIRST"}}
...
W ("decision="+$decision) $(if($decision -eq "GO_OPERATOR_V2_HEALTHY"){"Green"}else{"Red"})
W ("readiness="+$readiness) Cyan
W ("screens="+$screens.Count+" actions_ready="+$actionsReady+" actions_planned="+$actionsPlanned) Gray
...
if($decision -ne "GO_OPERATOR_V2_HEALTHY"){exit 12}else{exit 0}
