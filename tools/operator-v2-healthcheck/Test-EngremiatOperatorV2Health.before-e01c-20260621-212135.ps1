...
$badChildren=@();$plannedChildren=@();$criticalActive=@("inicio","pantallas_ux","ux_registry","screen_states","layout_rules","pending_ux_repairs","progreso_pendientes");foreach($s in $screens){foreach($cid in @($s.children)){if(!$ids.ContainsKey([string]$cid)){if($criticalActive -contains [string]$s.id){$badChildren+=([string]$s.id+"->"+[string]$cid)}else{$plannedChildren+=([string]$s.id+"->"+[string]$cid)}}}}
if($badChildren.Count -gt 0){$errors+="bad_critical_children:"+($badChildren -join ",")}else{$ok+="critical_children_links_ok"}
if($plannedChildren.Count -gt 0){$warn+="planned_child_stubs="+$plannedChildren.Count}else{$ok+="no_planned_child_stubs"}
...
$readiness=100;if($errors.Count -gt 0){$readiness-=60};if($warn.Count -gt 0){$readiness-=10};if($plannedChildren.Count -gt 0){$readiness-=5};if($readiness -lt 0){$readiness=0}
$decision=if($errors.Count -eq 0){"GO_OPERATOR_V2_HEALTHY_WITH_WARNINGS"}else{"NO_GO_OPERATOR_V2_HEALTHCHECK_FAILED"}
$report=[ordered]@{created_at=(Get-Date).ToString("s");package_id="ENGREMIAT_OPERATOR_V2_HEALTHCHECK_004";stage="E01B_REPAIR_HEALTHCHECK_PLANNED_CHILDREN_AS_WARN";decision=$decision;readiness=$readiness;active_root=$root;registry=$registry;engine=$engine;screens=$screens.Count;actions_ready=$actionsReady;actions_planned=$actionsPlanned;planned_child_stubs_count=$plannedChildren.Count;planned_child_stubs_sample=@($plannedChildren|Select-Object -First 20);errors=$errors;warnings=$warn;ok=$ok;next=if($errors.Count -eq 0){"E02_CLOSE_HEALTHCHECK_AND_RETURN_TO_ACTIONS_003"}else{"REPAIR_HEALTHCHECK_ERRORS_FIRST"}}
...
W ("screens="+$screens.Count+" actions_ready="+$actionsReady+" actions_planned="+$actionsPlanned+" planned_child_stubs="+$plannedChildren.Count) Gray
