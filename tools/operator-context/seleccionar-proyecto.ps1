...
if(!$InputValue){Write-Host "PROYECTOS DISPONIBLES" -ForegroundColor Cyan;$i=0;$items|ForEach-Object{$i++;Write-Host (" [{0}] {1}" -f $i,$_.name)};$InputValue=Read-Host "Numero de proyecto"}
...
if(!$p){throw "Proyecto no encontrado. Usa: powershell -File tools\operator-context\seleccionar-proyecto.ps1 -InputValue 1"}
