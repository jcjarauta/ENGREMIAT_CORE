...
# AUTO_MAINTENANCE_CARD_REAL_INICIO_PROMPT_E05I_BEGIN
$__eng_inicio_cmd = [string]$c
if ($__eng_inicio_cmd.Trim().ToLowerInvariant() -eq "m") {
  Write-Host "==== TARJETA DE MANTENIMIENTO CONTEXTUAL ====" -ForegroundColor Cyan
  $sev2 = Read-Host "Severidad LOW/MEDIUM/HIGH/BLOCKER"
  $exp2 = Read-Host "Esperado"
  $obs2 = Read-Host "Observado / que falla"
  $act2 = Read-Host "Accion realizada antes del fallo"
  New-EngremiatMaintenanceCard -ScreenId "S02_INICIO" -Route "INICIO" -Expected $exp2 -Observed $obs2 -ActionDone $act2 -Severity $sev2 | Out-Null
  Read-Host "Enter para volver a INICIO" | Out-Null
  continue
}
# AUTO_MAINTENANCE_CARD_REAL_INICIO_PROMPT_E05I_END
if ($c -eq "m") {
  Write-Host "==== TARJETA DE MANTENIMIENTO CONTEXTUAL ====" -ForegroundColor Cyan
