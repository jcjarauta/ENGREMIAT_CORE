# E10M prepare stable launcher direct operator access no apply

## Decision
GO_STABLE_DIRECT_OPERATOR_ACCESS_PATCH_READY_NO_APPLY

## Reason
La ruta por INICIO y WORKERS PAUSADO no es estable para esta prueba. Se prepara acceso directo desde ENGREMIAT.ps1 al operador versionado, sin activar worker real y sin tocar INICIO.
...
## Context after planned repair
   ...
   11: if ($op -eq "6") {
   12:   $operatorLauncher = Join-Path $PSScriptRoot "tools\desktop-terminal-operator\Start-EngremiatDesktopOperator.ps1"
   13:   if (Test-Path $operatorLauncher) {
   14:     & powershell -NoProfile -ExecutionPolicy Bypass -File $operatorLauncher
   15:   } else {
   16:     Write-Host "Operador local no encontrado: $operatorLauncher" -ForegroundColor Yellow
   17:   }
   18:   return
   19: }

## Rule
NO_APPLY. No modifica launcher real, INICIO, operador, adapter ni git. No activa worker real.

## Next
E10N_APPLY_STABLE_DIRECT_OPERATOR_ACCESS_WITH_GATE_BACKUP_SMOKE
