...
# SCREEN_MASTER_TEMPLATE_CANDIDATE_E11_BEGIN
# NO_APPLY candidate generated by ENGREMIAT_SCREEN_MASTER_TEMPLATE_001.
# Purpose: add a reusable contextual footer for manual screen maintenance.
# Apply only after human gate and backup.

function Open-EngContextMaintenanceCard {
  Clear-Host
  Write-Host "==== TARJETA DE MANTENIMIENTO CONTEXTUAL ====" -ForegroundColor Cyan
  Write-Host "Ruta: ENG_CONTEXT" -ForegroundColor DarkCyan
  ...
  if(Get-Command New-EngremiatMaintenanceCard -ErrorAction SilentlyContinue){
    New-EngremiatMaintenanceCard -ScreenId "ENG_CONTEXT" -Route "ENG_CONTEXT" -Expected $exp -Observed $obs -ActionDone $act -Severity $sev | Out-Null
  } else {
    Write-Host "WARN New-EngremiatMaintenanceCard no disponible en este contexto" -ForegroundColor Yellow
  }

  Read-Host "Enter para volver" | Out-Null
}

function Show-EngContextScreenFooter {
  while($true){
    Write-Host ""
    Write-Host "[?] Ayuda" -ForegroundColor White
    Write-Host "[b/q] volver | m = mantenimiento | Enter = refrescar" -ForegroundColor DarkGray
    $cmd = Read-Host "ENG_CONTEXT"
    if([string]::IsNullOrWhiteSpace($cmd)){ return }

    switch($cmd.Trim().ToLowerInvariant()){
      "m" { Open-EngContextMaintenanceCard }
      "?" {
        Write-Host ""
        Write-Host "Ayuda ENG_CONTEXT" -ForegroundColor Cyan
        Write-Host "Enter = refrescar / volver al contexto" -ForegroundColor Gray
        Write-Host "b/q = salir" -ForegroundColor Gray
        Write-Host "m = crear tarjeta contextual" -ForegroundColor Gray
...
Show-EngContextScreenFooter
# SCREEN_MASTER_TEMPLATE_CANDIDATE_E11_END
